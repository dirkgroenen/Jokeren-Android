...
public class Game extends Activity {
	...
	private ImageView ivDeckClosed, ivDeckOpen, ivPlayerCard1, ivPlayerCard2,
			ivPlayerCard3, ivPlayerCard4, ivPlayerCard5, ivPlayerCard6,
			ivPlayerCard7, ivPlayerCard8, ivPlayerCard9, ivPlayerCard10,
			ivPlayerCard11, ivPlayerCard12, ivPlayerCard13, ivGrabbedCard;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().getExtras() != null) {
			gameData = (GameData) getIntent().getExtras().getSerializable(
					GAME_DATA);
			Log.i("CORE",
					"Gamedata gets set with data received from mainscreen");
		} else {
			...
			public void onClick(View arg0) {
				if (!gameData.getGrabbedCard()) {
					...
					Toast.makeText(
							...
							getResources().getString(R.string.grabbedCardError),
	...
	private void initGraphics() {
		...
		playerCards = new ImageView[] { ivPlayerCard1, ivPlayerCard2, ivPlayerCard3, ivPlayerCard4, ivPlayerCard5, ivPlayerCard6,ivPlayerCard7, ivPlayerCard8, ivPlayerCard9, ivPlayerCard10,ivPlayerCard11, ivPlayerCard12, ivPlayerCard13, ivGrabbedCard };
	}
	...
	private void dealCards() {
		...
		ivDeckOpen.setImageResource(gameData.getDeck().openTopCard()
				.getImageResourceId());
	...
	private void markPlayerCardSelected(int cardIndex) {
		...
		int leftmargin = (card.getId() != R.id.ivgrabbedcard) ? -Math.round((float) 25 * density) : 0;
...
/*
	private void redrawHand() {
		...
		ImageView[] cardView = hand.getCardsView();
		for (int index = 0; index < 14; index++) {
			...
			if (card != null) {
				...
				cardView[index].setImageResource(card.getImageResourceId());

				// Convert DP to PX for margin
				float density = getApplicationContext().getResources()
						.getDisplayMetrics().density;
				int leftmargin = (image.getId() != R.id.ivgrabbedcard) ? -Math
						.round((float) 25 * density) : 0;

				// Make sure that the card is down again
				LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
						image.getLayoutParams().width,
						image.getLayoutParams().height, 1);
